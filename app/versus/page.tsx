...
export default function VersusPage() {
  ...
  const fetchVersusGames = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/versus?player1=${player1Id}&player2=${player2Id}`);
      const data = await response.json();
      setGames(data.games);
  ...
  const calculateStats = (games: Game[], p1Id: number, p2Id: number): VersusStats => {
    let player1Wins = 0;
    let player2Wins = 0;

    games.forEach(game => {
      const player1Team = game.playerGames.find(pg => pg.player.id === p1Id)?.team;
      const player2Team = game.playerGames.find(pg => pg.player.id === p2Id)?.team;
      
      if (player1Team && player2Team) {
        const isTeamAWin = game.scoreTeamA > game.scoreTeamB;
        if ((player1Team === 'A' && isTeamAWin) || (player1Team === 'B' && !isTeamAWin)) {
          player1Wins++;
        } else {
          player2Wins++;
        }
    ...
    <div className="max-w-4xl mx-auto p-4">
        ...
        <div className="bg-white rounded-xl shadow-md p-4 sm:p-6 mb-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:space-x-4">
              <Image
                src={players.find(p => p.id === player1Id)?.profileImage || '/default-profile.png'}
            ...
            <div className="self-center border-t border-b sm:border-0 py-3 sm:py-0 my-3 sm:my-0">
              <div className="text-base sm:text-lg font-bold">{stats.totalGames}경기</div>
            ...
            <div className="flex flex-col-reverse sm:flex-row items-center sm:space-x-4">
              ...
              <Image
                src={players.find(p => p.id === player2Id)?.profileImage || '/default-profile.png'}
      ...
      {games.length > 0 && (
        <div className="space-y-4">
          {games.map((game) => {
            // 승패 계산
            const player1Team = game.playerGames.find(pg => pg.player.id === player1Id)?.team;
            const isTeamAWin = game.scoreTeamA > game.scoreTeamB;
            const isPlayer1Win = (player1Team === 'A' && isTeamAWin) || (player1Team === 'B' && !isTeamAWin);
              ...
              <div key={game.id} className="bg-white rounded-xl shadow-sm p-6">
                ...
                <div className="grid grid-cols-3 gap-4 items-center mb-4">
                  ...
                  <div className="text-center">
                    <div className="text-sm font-medium text-gray-500 mb-1">A팀</div>
                    <div className="font-bold text-2xl mb-2">
                      {game.playerGames.find(pg => pg.player.id === player1Id)?.team === 'A'
                        ? game.scoreTeamA
                        : game.scoreTeamB}
                    </div>
                        ...
                        .map(pg => (
                          ...
                          >
                            ...
                            {pg.player.id === player1Id && (
                              <span className={`ml-1 ${isPlayer1Win ? 'text-blue-600' : 'text-red-600'}`}>
                                {isPlayer1Win ? '(승)' : '(패)'}
                              </span>
                            )}
                  ...
                  {/* Team B */}
                  <div className="text-center">
                    <div className="text-sm font-medium text-gray-500 mb-1">B팀</div>
                    <div className="font-bold text-2xl mb-2">
                      {game.playerGames.find(pg => pg.player.id === player2Id)?.team === 'A'
                        ? game.scoreTeamA
                        : game.scoreTeamB}
                    </div>
                    <div className="text-sm text-gray-600">
